...
import tcn_sequence_models.utils.train_split
...
class Preprocessor:
    ...
    def process(
        ...
        X_encoder = pad_sequences(X_encoder, maxlen = self.input_seq_len,padding='post', truncating='post', dtype='float32')
    ...
    def save_preprocessor_config(self, save_path):
        ...
        config_dict["time_col"] = self.time_col.tolist()
    ...
    def train_split(self):
        # print('train:', self.y.shape)
        (
            X_train,
            y_train,
        ) = tcn_sequence_models.utils.train_split.train_test_split(
            self.X, self.y
        )
