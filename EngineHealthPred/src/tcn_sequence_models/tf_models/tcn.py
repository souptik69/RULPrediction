...
class TCN(Model):
    ...
    def build(self, input_shape):
        ...
        #     )
        if self.num_layers is None:
            self.num_layers = max(1, math.ceil(
                math.log(
                    (
                        (self.max_seq_len - 1)
                        * (self.dilation_base - 1)
    ...
    def call(self, inputs, training=None):
        # tf.print("Shape before processing:", tf.shape(inputs))
        x = inputs
        for res_block in self.res_block_list:
            x = res_block(x, training=training)
        # tf.print("Shape after processing:", tf.shape(x))
        # x_last = x[:, -1, :]
        # x_last = tf.keras.layers.Lambda(lambda x: x[:, -1, :])(x)
        if self.return_sequence:
            return x
        else:
            x_last = x[:, -1, :]
            # x_last = tf.gather(x, self.max_seq_len - 1, axis=1)
            # tf.print("Shape after return sequencing:", tf.shape(x_last))
            return x_last
