...
from .decodertransfer import Decoder
from .encoder import Encoder


class TCN_TCN_attention_transfer(tf.keras.Model):
    def __init__(
        self,
        num_filters: int,
        kernel_size: int,
        ...
        step are not used.
        """
        super(TCN_TCN_attention_transfer, self).__init__()
    ...
    def build(self, input_shape):
        ...
        # Freeze all layers except the last two dense layers in the decoder
        if self.transfer == True:
            for layer in self.encoder.layers:
                layer.trainable = False
            for layer in self.decoder.layers[:-3]:
                layer.trainable = False
        else:
            for layer in self.encoder.layers:
