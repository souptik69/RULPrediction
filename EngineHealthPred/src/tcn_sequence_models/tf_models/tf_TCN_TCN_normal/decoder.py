...
class Decoder(tf.keras.Model):
    ...
    def _call_none_regressive(self, inputs, training=None):
        ...
        # data_encoder = tf.expand_dims(data_encoder, -1)
        ...
        # encoder_last_tiled = tf.expand_dims(encoder_last_tiled,-1)
    ...
    def _training_call_autoregressive(self, inputs):
        ...
        #Concatenating with y_shifted and passing through the decoder
        data_decoder_mod = self.random_feature_mask(data_decoder)
        # y_shifted = self.random_feature_mask(y_shifted)
        # y_shifted = tf.expand_dims(y_shifted, -1)
        # data_decoder_mod = tf.expand_dims(data_decoder_mod, -1)
        data_decoder_mod = tf.concat([data_decoder_mod, y_shifted], -1)
        # data_decoder_mod= tf.reshape(data_decoder_mod,shape=(tf.shape(data_decoder_mod)[0],tf.shape(data_decoder_mod)[1],(tf.shape(data_decoder_mod)[2])*2))
        out_tcn = self.tcn1(data_decoder_mod, training=True)
        encoder_last = data_encoder[:, -1:, :]
        encoder_last_tiled = tf.tile(encoder_last, [1, data_decoder_mod.shape[1], 1])
        #encoder_last_tiled = tf.expand_dims(encoder_last_tiled,-1)
        out = tf.concat([out_tcn, encoder_last_tiled], axis=-1)
